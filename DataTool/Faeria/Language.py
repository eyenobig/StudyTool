...
def clearjson(data):
    for x in data:
        for k in x["config"]:
            if x["config"][k].find("'") > -1:
                x["config"][k] = x["config"][k].replace("'","$$$")
            if x["config"][k].find("\xa0") > -1:
                x["config"][k] = x["config"][k].replace("\xa0"," ")
    return str(data).replace("'",'"').replace("$$$","'")


with open("jsonlang.json", "w" , encoding="utf-8") as f:
    f.write(clearjson(resjson))
# a = json.loads(clearjson(resjson))
# print(a)
